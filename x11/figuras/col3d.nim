...
var
  ...
  graphicsContext: GC              # Contexto gráfic
  screen: cint                     # Variable global para la pantalla de X11
...
# Procedimiento para dibujar el cubo
proc drawCube() =
  let
  ...
  var colorStruct = XColor()  # Crear una estructura XColor para almacenar el resultado
  let redColor = XParseColor(display, DefaultColormap(display, screenNum), "#FF0000", addr colorStruct)
  discard XAllocColor(display, DefaultColormap(display, screenNum), redColor)
  XSetForeground(display, graphicsContext, redColor.pixel)
...
proc mainLoop() =
  ...
  while true:
    ...
    of Expose:
      ...
      drawCube()                  # Redibujar el cubo cuando la ventana se expone
    of ClientMessage:
    ...
    of KeyPress:
      ...
      discard XClearWindow(display, window)
      drawCube()                  # Redibujar el cubo después de manejar una tecla
    else:
      discard
