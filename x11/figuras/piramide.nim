...
# Define los vértices y aristas del cubo
var
  vertices = [
    (x: -1.0, y: -1.0, z: -1.0),
    (x:  1.0, y: -1.0, z: -1.0),
...
# Define los vértices y aristas del tetraedro
var
  tetraVertices = [
    (x: 0.0, y: 1.0, z: 0.0),    # Vértice 0
    (x: -0.5, y: 0.0, z: -sqrt(3.0)/2),  # Vértice 1
    (x: 0.5, y: 0.0, z: -sqrt(3.0)/2),   # Vértice 2
    (x: 0.0, y:0.0, z: sqrt(3.0)/2)     # Vértice 3
  ]
  
  tetraEdges = [
    (0, 1), (0, 2), (0, 3),   # Aristas desde el vértice 0
    (1, 2), (2, 3), (3, 1)    # Aristas entre vértices 1, 2, 3
  ]


# Procedimiento para dibujar el tetraedro
proc drawTetrahedron() =
  let
  ...
  for (i, j) in tetraEdges:
    var
      v1 = matMulVec(rotationX, matMulVec(rotationY, tetraVertices[i]))
      v2 = matMulVec(rotationX, matMulVec(rotationY, tetraVertices[j]))
    # Aplicar translación
...
proc mainLoop() =
  ...
  while true:
    ...
    of Expose:
      ...
      drawTetrahedron()           # Dibujar el tetraedro
    of ClientMessage:
    ...
    of KeyPress:
      ...
      discard XClearWindow(display, window)
      drawTetrahedron()           # Redibujar el tetraedro después de manejar una tecla
    else:
      discard
